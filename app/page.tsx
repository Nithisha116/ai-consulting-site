    ...
    <main ref={containerRef} className="min-h-screen bg-[#F9F9FB] text-[#111115] selection:bg-indigo-100 relative overflow-x-hidden">
      ...
      <section ref={scrollSceneRef} className="relative w-full h-screen flex items-center overflow-hidden bg-black text-white px-6 lg:px-16 will-change-transform">
        ...
        >
          ...
          <video 
            ref={videoRef}
            className="w-full h-full object-cover opacity-60 will-change-transform origin-center"
            loop 
            muted 
            playsInline
          >
        ...
        <motion.div 
          ref={overlayGlowRef}
          className="absolute inset-0 pointer-events-none z-10 opacity-0 mix-blend-screen bg-[radial-gradient(circle_at_35%_45%,rgba(99,102,241,0.22),transparent_55%)]"
        ...
        <div className="absolute inset-0 z-10 pointer-events-none overflow-hidden">
          {particles.map((p) => (
            <div
              key={p.id}
              className="absolute rounded-full will-change-transform opacity-25"
              style={{
                left: `${p.x}%`,
                top: `${p.y}%`,
                width: `${p.size}px`,
        ...
        >
          ...
          <div ref={btnGroupRef} className="opacity-0 flex flex-wrap gap-4 will-change-transform">
            <motion.a 
              href="#audit" 
              className="px-6 py-3.5 bg-indigo-600 text-white font-bold text-xs uppercase tracking-wider rounded-xl shadow-lg flex items-center gap-2 group border border-indigo-500 origin-center"
              whileHover={{ scale: 1.03, y: -3, rotate: 0.5, boxShadow: "0px 8px 24px rgba(99,102,241,0.35)", borderColor: "#818cf8" }}
              whileTap={{ scale: 0.98 }}
            ...
            <motion.a 
              href="#sculpture" 
              className="px-6 py-3.5 bg-white/10 backdrop-blur-md border border-white/20 text-white font-bold text-xs uppercase tracking-wider rounded-xl hover:bg-white/20 transition-all origin-center"
